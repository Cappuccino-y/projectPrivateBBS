...
const CommentItem = ({comment, handleDelete}) => {
    ...
    useEffect(() => {
        if (editMode) {
            if (updateBlogRef.current && val.blog.comments.length === updateBlogRef.current.comments.length)
                val.setBlog(updateBlogRef.current);
        }
    }, [val.blogs]);
    ...
    return (<Card
        ...
        >
            ...
            <div style={{height: '30%', display: 'flex', flexDirection: 'column'}}>
                <Paper
                    style={{
                        display: val.commentShow ? 'flex' : 'none',
